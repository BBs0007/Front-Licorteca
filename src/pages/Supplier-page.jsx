...
class SupplierPage extends Component {
    ...
    peticionGet = () => {
        axios.get(url).then(response => {
            this.setState({data: response.data.suppliers});
        }).catch(error => console.log(error.message));
    }

    peticionPost = async () => {
        await axios.post(url, {supplier: this.state.form}).then(response => {
            this.modalInsertar();
            this.peticionGet();
        }).catch(error => console.log(error.message));
    }
    ...
    peticionPut = () => {
        axios.put(url+`/${this.state.form.id}`, {supplier: this.state.form}).then( response => {
            this.modalInsertar();
            this.peticionGet();
        }).catch(error => console.log(error.message));
    }
            ...
            <>
                <Box sx={{display: 'flex'}}>
                    ...
                    <Box component="main" sx={{flexFlow: 1, p: 3}}>
                        <div className='App'>
                            <br/>
                            <button className='btn btn-success' onClick={ () => {this.setState({form: {}, typeModal: 'insert'}), this.modalInsertar()}}>Add Supplier</button>
                            <br/> <br/>
                            ...
                            <tbody>
                                ...
                                this.state.data.map((supplier) => {
                                    ...
                                    <tr key={supplier.id}>
                                        ...
                                        <td>
                                        <button className='btn btn-primary' onClick={() => {this.selectSupplier(supplier); this.modalInsertar()}}><FontAwesomeIcon icon={faEdit}/></button>
                                        {"    "}
                                        <button className='btn btn-danger'  onClick={() => {this.selectSupplier(supplier); this.setState({modalEliminar: true})}}><FontAwesomeIcon icon={faTrashAlt}/></button>
                                        </td>
                                    </tr>
                                    )
                                })
